...
export default function ProductDetails({
  ...
  const handleAddToCart = async () => {
    try {
      const Response = await axios.post(`${process.env.NEXT_PUBLIC_HOST}/api/order_app/cart/create/${user?.id}/?productId=${product?.id}&qty=${qty}`)
      setIsCartOpen(true);
    } catch (err) {
      console.error(err)
    }
                ...
                <div className="flex flex-col items-center p-4 mb-36">
                  ...
                  <div className="bg-white-A700 border-2 border-gray-200 rounded-lg px-4 self-start py-4">
                    ...
                    <div className="flex items-center mt-2">
                      ...
                      <select value={qty}
                        onChange={(event:any)=>setQty(event.target.value)} className="ml-2 w-full border border-gray-300 rounded p-1">
